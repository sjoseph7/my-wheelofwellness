...
<?php
    ...
    if(filter_has_var(INPUT_POST,'submit')) {
        ...
        if (allElementsAreNull($selection_Array) && is_null($noConcern)){
            ...
            $logFile = fopen("patientSelectionLog.csv", "a");
            fwrite( 
                $logFile, 
                $date           . ", " . 
                $study_ID       . ", " .
            ...
            $subject = "New Patient Concerns Submitted!";
            $txt = "A new 'child/adolescent' submission has been logged for Study ID#: $study_ID";
            ...
            echo '<!--Written by Senghor Joseph - Weekend Websites - Copyright © 2018-->
                ...
                <body>
                    ...
                    <main id="main">
                        ...
                        for ($i = 0; $i < min(count($selection_Array),count($concernArray['patientConcerns'])); $i++){
                            if (!is_null($selection_Array[$i])){
                                // Insert Concern "More Info" Link
                                echo '<div class="response">
                                        <div class="response-container" onclick="moreInfo(\''.$concernArray['patientConcerns'][$i]['link'].'\')">
                                            <h4><b>';
                                // Insert Concern Title
                                echo $concernArray['patientConcerns'][$i]['concern'];
                                echo '</b></h4><p>';
                                // Insert Concern Info
                                echo $concernArray['patientConcerns'][$i]['info'];
                                echo '</p>
                                </div>
                                </div>';
                            }
...
<body>
    ...
    <main id="main">
        ...
        <section id="section-c" class="grid">
            ...
            <form method="post" action="patient-concerns.php">
                    ...
                    <table>
                        <tr>
                            <td>
                                ...
                                <label class="container" onclick="uncheckNoConcern()">What does the HPV vaccine protect against?
                                    <input name="chkbx_3" value="yes" type="checkbox" <?php if (!is_null($selection_Array[2])){echo "checked";}?>>
                                    <span class="checkmark"></span>
                                </label>

                                <label class="container" onclick="uncheckNoConcern()">How many shots will I need?
                                    <input name="chkbx_4" value="yes" type="checkbox" <?php if (!is_null($selection_Array[3])){echo "checked";}?>>
                                    <span class="checkmark"></span>
                                </label>

                                <label class="container" onclick="uncheckNoConcern()">Is HPV the same as HIV and HSV?
                                    <input name="chkbx_5" value="yes" type="checkbox" <?php if (!is_null($selection_Array[4])){echo "checked";}?>>
                                    <span class="checkmark"></span>
                                </label>

                                <label class="container" onclick="uncheckNoConcern()">Will the vaccine protect me against other sexually transmitted infections?
                                    <input name="chkbx_6" value="yes" type="checkbox" <?php if (!is_null($selection_Array[5])){echo "checked";}?>>
                                    <span class="checkmark"></span>
                                </label>

                                <label class="container" onclick="uncheckNoConcern()">What if I miss my next shot?
                                    <input name="chkbx_7" value="yes" type="checkbox" <?php if (!is_null($selection_Array[6])){echo "checked";}?>>
                                    <span class="checkmark"></span>
                                </label>

                                <label class="container" onclick="uncheckNoConcern()">Are my friends getting the shot?
                                    <input name="chkbx_8" value="yes" type="checkbox" <?php if (!is_null($selection_Array[7])){echo "checked";}?>>
                                    <span class="checkmark"></span>
                                </label>
                                <p style="color:red; padding:0"><?php if ($selectionAlertMessage !== ''){echo $selectionAlertMessage;} ?></p><br>
                                <label class="container" onclick="uncheckOthers()"><b>I have no concerns.</b>
                                    <input id="chkbx_NC" name="chkbx_NC" value="yes" type="checkbox" <?php if (!is_null($selection_Array[8])){echo "checked";}?>>
                                    <span class="checkmark"></span>
                                </label>
